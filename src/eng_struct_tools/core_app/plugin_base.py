...
from .i18n_manager import TranslationDomain, get_i18n_manager
...
class HostAPI:
    ...
    such as logging, configuration, UI integration, data exchange, and i18n.
    """

    def __init__(self, main_window, config_manager, logger, i18n_manager=None):
        """
    ...
    def translate(
        self, message: str, domain: Optional[str] = None, context: Optional[str] = None
    ) -> str:
        """
        Translate a message using the i18n system.

        Args:
            message: Message to translate
            domain: Translation domain (uses plugin domain if None)
        ...
        if self.i18n_manager:
            if domain is None:
                domain = "eng_struct_tools"  # Default domain
            return self.i18n_manager.translate(message, domain, context)
        return message
    ...
    def ngettext(
        ...
        if self.i18n_manager:
            if domain is None:
                domain = "eng_struct_tools"  # Default domain
            return self.i18n_manager.ngettext(singular, plural, n, domain)
        return singular if n == 1 else plural

    def get_current_locale(self) -> str:
        """Get the current locale code."""
        if self.i18n_manager:
            return self.i18n_manager.get_locale()
        return "en_US"
    ...
    def format_number(self, number: float, format_type: str = "decimal") -> str:
        ...
        """
        if self.i18n_manager:
            return self.i18n_manager.format_number(number, format_type)
        return str(number)
    ...
    def format_datetime(self, dt, format_type: str = "medium") -> str:
        ...
        """
        if self.i18n_manager:
            return self.i18n_manager.format_datetime(dt, format_type)
        return str(dt)

    def is_rtl_locale(self) -> bool:
        """Check if current locale uses right-to-left text direction."""
        if self.i18n_manager:
            return self.i18n_manager.is_rtl_locale()
        return False
...
class PluginBase(ABC):
    ...
    def __init__(self):
        ...
        self._translation_domain: Optional[TranslationDomain] = None
    ...
    def setup_translation_domain(self, plugin_dir: Path) -> None:
        """
        Set up translation domain for this plugin.

        Args:
            plugin_dir: Plugin directory path
        """
        if self.plugin_info:
            domain_name = self.plugin_info.name.lower().replace(" ", "_")
            locale_dir = plugin_dir / "locales"

            if locale_dir.exists():
                self._translation_domain = TranslationDomain(domain_name, locale_dir)

                # Register with global i18n manager
                i18n_manager = get_i18n_manager()
                if i18n_manager:
                    i18n_manager.register_domain(self._translation_domain)

    def get_translation_domain_name(self) -> Optional[str]:
        """Get the translation domain name for this plugin."""
        if self._translation_domain:
            return self._translation_domain.domain_name
        return None

    def _(self, message: str, context: Optional[str] = None) -> str:
        """
        Translate a message using this plugin's domain.
        ...
        """
        if self.host_api:
            domain = self.get_translation_domain_name()
            return self.host_api.translate(message, domain, context)
        return message

    def ngettext(self, singular: str, plural: str, n: int) -> str:
        """
        Translate with pluralization support using this plugin's domain.
        ...
        if self.host_api:
            domain = self.get_translation_domain_name()
            return self.host_api.ngettext(singular, plural, n, domain)
        return singular if n == 1 else plural

    def get_localized_plugin_info(self) -> PluginInfo:
        """
        Get plugin info with localized strings.

        Returns:
            PluginInfo with translated name and description
        """
        info = self.get_plugin_info()

        # Create a copy with translated strings
        localized_info = PluginInfo(
            name=self._(info.name),
            version=info.version,
            description=self._(info.description),
            author=info.author,
            category=self._(info.category),
